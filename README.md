...
src/
  app/
    ...
    api/                        # route handlers (comics, boards, meta, images)
  components/
    board/    # Masonry (layout + dnd), ComicCard, BoardTabs, BoardView
    ...
    ui/       # Dialog, Menu, TagInput, Autocomplete, Toast, icons
  db/         # Drizzle schema, client, queries, migrations, seed
  lib/        # storage adapter, image processing, fractional-index, filters
```
...
- **Fractional indexing** — a reorder writes a single row (`position` = midpoint
  of neighbors) instead of renumbering the board.
- **Storage is abstracted** behind `StorageAdapter` (local FS today) so S3/R2 is
  a drop-in later. All DB access goes through the API layer, so adding auth /
  multi-user later doesn't touch UI code.

## Roadmap

- Edit mode in the detail modal (reuses `MetadataForm`).
